...
const getVector = (matrix: Matrix, rowIndex: number) =>
  Array.from(matrix.values.slice(
...
describe("getPositionEncoding", () => {
  ...
  it("uses the same angle for each even and odd dimension pair", () => {
    ...
    for (let featureIndex = 0; featureIndex < 6; featureIndex += 2) {
      ...
      expect(sinValue ** 2 + cosValue ** 2).toBeCloseTo(1, 5);
    }
  });
  ...
  it("matches the sinusoidal formula for the first two dimension pairs", () => {
    ...
    expect(getValue(encoding, 1, 0)).toBeCloseTo(Math.sin(1), 5);
    expect(getValue(encoding, 1, 1)).toBeCloseTo(Math.cos(1), 5);
    expect(getValue(encoding, 1, 2)).toBeCloseTo(Math.sin(0.01), 5);
    expect(getValue(encoding, 1, 3)).toBeCloseTo(Math.cos(0.01), 5);
  });
});
