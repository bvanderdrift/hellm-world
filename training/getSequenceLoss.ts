...
export const getSequenceLoss = (
  ...
  const dim = activations.unembeddingsOutputLogits.dimensions;
  const outputProbabilities = createMatrix(
    activations.unembeddingsOutputLogits.vectors,
    dim,
  );
  const outputLosses = new Float32Array(
    activations.unembeddingsOutputLogits.vectors,
  );

  for (
    let contextIndex = 0;
    contextIndex < activations.unembeddingsOutputLogits.vectors;
    contextIndex++
  ) {
    const outputLogits = getRawVector(
    ...
    if (correctTokenIndex === -1) {
      outputProbabilities.values.set(
        outputLogits.map((_) => 0),
        contextIndex * dim,
      );
    ...
    const probabilities = safeLogits.map((logit) => Math.exp(logit) / summed);

    outputProbabilities.values.set(probabilities, contextIndex * dim);
    outputLosses[contextIndex] = baseAdjusted - correctTokenLogitAdjusted;
  }
