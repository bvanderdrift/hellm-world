...
const runGpuPacked = async (
  packedInput: Matrix,
  headsCount: number,
  attentionWeights: AttentionWeights,
): Promise<Matrix> => {
  const vectors = packedInput.vectors;
  const hiddenDimensions = packedInput.dimensions;
  ...
  const inputBuffer = createMatrixBuffer(packedInput);
...
describe("runSelfAttentionMechanismOnGPU — multi-batch", () => {
  it("matches per-batch CPU output for two single-head batches of different lengths", async () => {
    ...
    const expectedA = runSelfAttentionMechanism(batchA, headsCount, weights).output;
    const expectedB = runSelfAttentionMechanism(batchB, headsCount, weights).output;

    const actual = await runGpuPacked(packed, headsCount, weights);
  ...
  it("does not leak one batch's tokens into another batch's attention", async () => {
    ...
    const actualFirst = await runGpuPacked(
      packBatches([firstA, batchB]),
      headsCount,
      weights,
    );
    const actualSecond = await runGpuPacked(
